...
class UNetInferenceAgent:
    ...
    def single_volume_inference(self, volume):
        ...
        for idx in range(volume.shape[0]):
            sel_slice = volume[idx,:,:]
            proc_slice = sel_slice.astype(np.single)/255.
            slice_tensor = torch.from_numpy(proc_slice).unsqueeze(0).unsqueeze(0)
            prediction = self.model(slice_tensor.to(self.device))
            prediction =  np.squeeze(prediction.cpu().detach())
            slices[idx,:,:] = torch.argmax(prediction, dim=0)  
