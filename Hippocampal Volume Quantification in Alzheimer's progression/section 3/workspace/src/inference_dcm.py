...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text((10, 90),
                        ...
                        Type of image: {header.ImageType}\n \
                        Study: {header.StudyDescription}\t \
                        Series: {header.SeriesDescription}\n ================================\n \
    ...
    pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").resize((350, 350))
...
def get_series_for_inference(path):
    ...
    #series_path = [dir for dir, subdirs, files in os.walk(path) if 'HCropVolume' in dir] #from knowladge Udacity
    
    
    #sel_pth = str(np.random.choice(series_path,size=int(len(series_path) > 0))) #select a random path
    
    #series_for_inference = [pydicom.dcmread(os.path.join(sel_pth, file)) for file in os.listdir(sel_pth)]
